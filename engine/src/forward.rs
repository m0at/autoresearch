...
pub fn forward(bufs: &mut BufferManager, gemm: &GemmRunner) {
    ...
    for layer in 0..N_LAYER {
        ...
        // [LAYER STAT] Capture MLP output norm and apply dynamic importance scale
        unsafe {
            ffi::layer_l2_norm_bf16(
                dptr(&bufs.xn) as *const std::ffi::c_void,
                (bt * D_MODEL) as i32,
                dptr(&bufs.layer_act_norms) as *mut f32,
                layer as i32,
                stream,
            );
            ffi::layer_scale_bf16(
                dptr(&bufs.layer_dynamic_scale) as *const f32,
                layer as i32,
                dptr(&bufs.xn) as *mut std::ffi::c_void,
                (bt * D_MODEL) as i32,
                stream,
            );
        }
