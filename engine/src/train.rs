...
pub fn train(config: TrainConfig) -> Result<()> {
    ...
    let mut layer_scores: Vec<f64> = vec![1.0; N_LAYER];
    ...
    // Track when each layer was last reinitialized (0 = never).
    ...
    let mut layer_reinit_step: Vec<usize> = vec![0; N_LAYER];
    ...
    loop {
        ...
        if needs_log {
            ...
            // Update dynamic layer importance scores from GPU norms
            {
                let act_norms = read_f32_buf(&stream, &bufs.layer_act_norms);
                let grad_norms = read_f32_buf(&stream, &bufs.layer_grad_norms);
                for i in 0..N_LAYER {
                    // Generalization ratio: how much of this layer's gradient signal
                    // is reproducible on val data. High ratio → learning transferable
                    // features → safe to let it run. Low ratio → memorizing → suppress.
                    let gen_ratio = (layer_val_grad_norms_host[i] as f64
                        / (grad_norms[i] as f64 + 1e-8))
                        .clamp(0.1, 10.0); // clamp prevents explosion on first few steps
                    let signal = act_norms[i] as f64 * grad_norms[i] as f64 * gen_ratio;
                    layer_scores[i] = LAYER_SCORE_EMA * layer_scores[i] + (1.0 - LAYER_SCORE_EMA) * signal;
                }
                // Normalize so mean = 1.0, apply post-reinit boost, write to GPU.
                // Layers reinitialized within the last REINIT_BOOST_STEPS steps get a
                // 2× → 1× linearly decaying scale multiplier so fresh weights receive
                // stronger gradient signal while they're catching up.
                let mean_score: f64 = layer_scores.iter().sum::<f64>() / N_LAYER as f64;
                if mean_score > 1e-12 {
                    let scale_host: Vec<f32> = (0..N_LAYER).map(|i| {
                        let base = (layer_scores[i] / mean_score) as f32;
                        let steps_since_reinit = step.saturating_sub(layer_reinit_step[i]);
                        if layer_reinit_step[i] > 0 && steps_since_reinit < REINIT_BOOST_STEPS {
                            let decay = 1.0 - steps_since_reinit as f32 / REINIT_BOOST_STEPS as f32;
                            base * (1.0 + decay) // 2× at step 0, 1× at step REINIT_BOOST_STEPS
                        } else {
                            base
                        }
                    }).collect();
                    stream.memcpy_htod(&scale_host, &mut bufs.layer_dynamic_scale)
                        .expect("htod layer_dynamic_scale");
                }
            }
            ...
            if needs_eval {
                ...
                // Capture val gradient norms: run forward+backward on one val batch
                // to measure how much gradient signal each layer produces on held-out data.
                // Ratio of val_grad / train_grad = generalization score per layer.
        ...
        // Every 200 steps: reinitialize dead layers (score < RINSE_THRESHOLD * mean, default 0.5)
        if step > 0 && step % 200 == 0 && step < config.max_steps.unwrap_or(usize::MAX).saturating_sub(config.cooldown_steps) {
            ...
            let threshold = rinse_frac * mean_score;
            let mut reinit_count = 0usize;
            let d = D_MODEL;
            let s = INIT_SCALE * 3.0_f64.sqrt() * (d as f64).powf(-0.5);
            for i in 0..N_LAYER {
                if layer_scores[i] < threshold {
                    // Reinit MLP weights (wfc: Uniform(-s,s), wdn: zeros)
                    let wfc_data = uniform_bf16(MLP_DIM * d, s);
                    stream.memcpy_htod(&wfc_data, &mut bufs.layer_weights[i].wfc)
                        .expect("htod wfc reinit");
                    let wdn_zeros_bf16 = vec![half::bf16::ZERO; d * MLP_DIM];
                    stream.memcpy_htod(&wdn_zeros_bf16, &mut bufs.layer_weights[i].wdn)
                        .expect("htod wdn reinit");
                    // f32 masters (Muon reads these as source of truth)
                    let wfc_f32: Vec<f32> = wfc_data.iter().map(|x| x.to_f32()).collect();
                    stream.memcpy_htod(&wfc_f32, &mut bufs.layer_weights[i].wfc_f32)
                        .expect("htod wfc_f32 reinit");
                    let wdn_zeros_f32 = vec![0.0f32; d * MLP_DIM];
                    stream.memcpy_htod(&wdn_zeros_f32, &mut bufs.layer_weights[i].wdn_f32)
                        .expect("htod wdn_f32 reinit");
                    // Zero Muon momentum + second_momentum for wfc (mat=4) and wdn (mat=5)
                    // so stale optimizer state doesn't corrupt the fresh weights on next step
                    let wfc_mom_zeros = vec![0.0f32; MLP_DIM * d];
                    let wdn_mom_zeros = vec![0.0f32; d * MLP_DIM];
                    stream.memcpy_htod(&wfc_mom_zeros, &mut bufs.muon.momentum[i * 6 + 4])
                        .expect("htod wfc_mom reinit");
                    stream.memcpy_htod(&wdn_mom_zeros, &mut bufs.muon.momentum[i * 6 + 5])
                        .expect("htod wdn_mom reinit");
                    stream.memcpy_htod(&wfc_mom_zeros, &mut bufs.muon.second_momentum[i * 6 + 4])
                        .expect("htod wfc_smom reinit");
                    stream.memcpy_htod(&wdn_mom_zeros, &mut bufs.muon.second_momentum[i * 6 + 5])
                        .expect("htod wdn_smom reinit");
                    layer_scores[i] = mean_score;
                    layer_reinit_step[i] = step;
                    reinit_count += 1;
                }
            }
            if reinit_count > 0 {
                println!("[layer_reinit] step {step}: reinitialized {reinit_count} layers (threshold={threshold:.3})");
            }
        }
